...
import pandas as pd  # <--- INI YANG KETINGGALAN TADI BRO
...
selected_stock = st.sidebar.selectbox("Pilih Aset Kripto:", ("BTC-USD", "ETH-USD", "SOL-USD"))
...
def load_data(ticker):
    ...
    # FIX: Meratakan kolom jika formatnya MultiIndex (Masalah update yfinance baru)
    # Ini biar 'Close' jadi kolom biasa, bukan index bertingkat
...
# Sekarang data['Close'] sudah pasti aman karena sudah di-fix di fungsi load_data
...
with col1:
    st.metric(
        ...
        value=f"${float(last_price):,.2f}", # Dipaksa jadi float biar aman
        delta=f"{delta_percent:.2f}%"
    )
with col2:
    st.metric(
        label="Volume Transaksi", 
        value=f"{float(data['Volume'].iloc[-1]):,.0f}" # Dipaksa jadi float biar aman
    )
with col3:
